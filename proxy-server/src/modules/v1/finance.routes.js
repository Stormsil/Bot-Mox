...
const { RtdbCollectionRepository, readRtdbPath } = require('../../repositories/rtdb/rtdb-repository');
const { parseListQuery, applyListQuery, asyncHandler } = require('./helpers');

function createFinanceRoutes({ admin }) {
  ...
  router.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      const items = await operationsRepo.list();
  ...
  router.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      const operation = await operationsRepo.getById(parsedId.data.id);
      if (!operation) {
  ...
  router.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      const created = await operationsRepo.create(parsedBody.data, explicitId || undefined);
  ...
  router.patch(
    ...
    asyncHandler(async (req, res) => {
      ...
      const updated = await operationsRepo.patch(parsedId.data.id, parsedBody.data);
      if (!updated) {
        return res.status(404).json(failure('NOT_FOUND', 'Finance operation not found'));
      }
  ...
  router.delete(
    ...
    asyncHandler(async (req, res) => {
      ...
      const removed = await operationsRepo.remove(parsedId.data.id);
      if (!removed) {
        return res.status(404).json(failure('NOT_FOUND', 'Finance operation not found'));
      }
  ...
  router.get(
    ...
    asyncHandler(async (_req, res) => {
      const stats = await readRtdbPath(admin, RTDB_PATHS.finance.dailyStats);
  ...
  router.get(
    '/gold-price-history',
    asyncHandler(async (_req, res) => {
      const history = await readRtdbPath(admin, RTDB_PATHS.finance.goldPriceHistory);
