...
const env = {
  ...
  agentAuthSecret: String(process.env.AGENT_AUTH_SECRET || process.env.LICENSE_LEASE_SECRET || '').trim(),
  agentTokenTtlSeconds: toBoundedInt(process.env.AGENT_TOKEN_TTL_SECONDS, 60 * 60 * 24 * 30, 300, 60 * 60 * 24 * 180),
  agentPairingPublicUrl: String(process.env.AGENT_PAIRING_PUBLIC_URL || '').trim().replace(/\/+$/, ''),
  ...
  s3BucketArtifacts: String(process.env.S3_BUCKET_ARTIFACTS || process.env.S3_BUCKET_THEME_ASSETS || '').trim(),
  s3BucketThemeAssets: String(process.env.S3_BUCKET_THEME_ASSETS || process.env.S3_BUCKET_ARTIFACTS || '').trim(),
