...
async function initOtel(): Promise<void> {
  ...
  try {
    ...
    const [{ WebTracerProvider, BatchSpanProcessor }, { ZoneContextManager }, { registerInstrumentations }] =
      await Promise.all([
    ...
    uiLogger.info(`[otel] frontend enabled (service=${serviceName}, exporter=${exporterEndpoint || 'disabled'})`);
  } catch (error) {
