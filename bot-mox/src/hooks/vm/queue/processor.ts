...
function pickPrimaryVolumeFromConfig(
  ...
  try {
    ...
    if (settings.storage?.autoSelectBest) {
      ...
      try {
        ...
        const normalizedEnabledTargets = enabledTargets.filter((name) =>
          freeBytesByStorage.has(name),
        );
        const pool = (
          normalizedEnabledTargets.length > 0 ? normalizedEnabledTargets : statsCandidates
        ).filter(Boolean);
        remainingFree = new Map(
          pool.map((name) => [name, freeBytesByStorage.get(name) ?? 0] as const),
        );
      } catch (error) {
        const msg = error instanceof Error ? error.message : 'Unknown error';
      ...
      try {
        ...
        const effectiveStorage =
          ...
          (() => {
            ...
            return configuredDefault.toLowerCase() === LEGACY_STORAGE_PLACEHOLDER
              ? ''
              : configuredDefault;
          })() ||
          'data';
    ...
    for (const { item, vmId } of clonedItems) {
      ...
      try {
        ...
        while (Date.now() < readDeadline && !cancelRef.current) {
          readAttempts++;
          try {
            const candidateConfig = (await getVMConfig(vmId, targetNode)) as Record<
              string,
              unknown
            >;
            ...
            log.warn(
              `VM ${vmId}: config read failed (attempt ${readAttempts}): ${readMsg}`,
              item.name,
            );
          }
          await sleep(2000);
        ...
        if (mutablePatchFields.length > 0) {
          ...
          if (!spoofTask.upid || String(spoofTask.upid).trim() === '') {
            ...
            log.taskLog(
              vmTaskKey,
              `Mutable patch task finished (${spoofStatus.exitstatus || 'OK'})`,
            );
          }
          log.taskLog(
            vmTaskKey,
        ...
        const desiredDiskGiB =
          Number.isFinite(customDiskGiB) && customDiskGiB > 0
            ...
            : resourceMode === 'project' &&
                Number.isFinite(configuredDiskGiB) &&
                configuredDiskGiB > 0
        ...
        if (desiredDiskGiB && primaryVolume && currentDiskGiB > 0) {
          ...
          if (incrementGiB > 0) {
            ...
            if (!resizeTask.upid || String(resizeTask.upid).trim() === '') {
              log.warn(
                `VM ${vmId}: resize returned no UPID, waiting briefly for convergence`,
                item.name,
              );
              await sleep(1500);
            } else {
          ...
          log.taskLog(
            vmTaskKey,
            'Disk resize skipped: cannot detect primary volume key/size',
            'warn',
          );
        }
        ...
        const diskPlanText = (() => {
          if (currentDiskGiB <= 0) {
            return desiredDiskGiB
              ? `${desiredDiskGiB} GiB (desired; current unknown)`
              : '(unknown)';
          }
          if (!desiredDiskGiB) {
        ...
        const waitForResourceConvergence = async (
          ...
          while (Date.now() < deadline && !cancelRef.current) {
            ...
            try {
              ...
              const msg = error instanceof Error ? error.message : 'Unknown error';
              log.warn(
                `VM ${vmId}: failed to read config during resource verify: ${msg}`,
                item.name,
              );
            }
        ...
        while (Date.now() < finalReadDeadline && !cancelRef.current) {
          ...
          try {
            ...
            const finalReadMsg =
              finalReadErr instanceof Error ? finalReadErr.message : 'Unknown error';
            log.warn(
        ...
        if (!finalApiConfig || !finalText) {
          throw new Error(
            `Could not read final VM ${vmId} config after ${finalReadAttempts} attempts`,
          );
        }
        log.taskLog(
          vmTaskKey,
    ...
    if (completedVmIds.length > 0 && !cancelRef.current) {
      ...
      for (const { item, vmId } of clonedItems) {
        ...
        try {
          ...
          log.info(
            `VM ${vmId}: ISO payload generated (${Object.keys(files).length} files)`,
            item.name,
          );
