...
export {
  deleteThemeAsset,
  type ListThemeAssetsResponse,
  listThemeAssets,
  type ThemeBackgroundAsset,
  uploadThemeAsset,
} from './themeAssetsFacade';

const THEME_SETTINGS_PATH = 'theme';
...
export const getThemeSettings = async (): Promise<ThemeSettings> => {
  try {
    const rawData = await readSettingsPath<unknown>(THEME_SETTINGS_PATH);
...
export const updateThemeSettings = async (
  ...
  await writeSettingsPath(THEME_SETTINGS_PATH, payload);
};
