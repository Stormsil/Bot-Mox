# Repository Architecture + De-Legacy Audit (2026-02-19)
...
3. De-legacy scan across code, scripts, docs, and DB tooling.
...
## Legacy Findings Table

| Path | Type | Severity | Finding | Action |
|---|---|---|---|---|
| `docs/plans/*` (historical) | Documentation | Medium | Legacy backend/path references | Moved to `docs/history/plans/*` |
| `docs/audits/*` (historical) | Documentation | Medium | Legacy migration references | Moved to `docs/history/audits/*` |
...
| `scripts/check-legacy-naming-cutover.js` | Guardrail script | Medium | Missing one legacy-path blocking rule | Updated in this hardening wave |
...
4. Enforce no-legacy naming in active files through strict guardrails.
