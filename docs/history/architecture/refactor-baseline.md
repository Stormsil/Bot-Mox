...
## Runtime topology
- Frontend app: `bot-mox/` (React + Refine + Vite)
- Backend server: `proxy-server/server.js` (legacy monolith + newly mounted `/api/v1`)
...
- `proxy-server/server.js` remains monolithic and should be split into `src/app.ts` + modules.
...
- API-first polling services introduced:
  ...
  - `bot-mox/src/services/botsApiService.ts`
  - `bot-mox/src/services/resourcesApiService.ts`
  - `bot-mox/src/services/resourceTreeSettingsService.ts`
- UI files migrated away from direct `firebase/database` imports:
  - `bot-mox/src/pages/dashboard/index.tsx`
  - `bot-mox/src/pages/datacenter/index.tsx`
  - `bot-mox/src/components/layout/Header.tsx`
  - `bot-mox/src/components/layout/ResourceTree.tsx`
- Metric delta:
...
- Added API-first resource domain service for licenses:
  - `bot-mox/src/services/licensesApiService.ts`
- Extended generic resources API service with CRUD operations:
  - `createResource`, `updateResource`, `deleteResource` in `bot-mox/src/services/resourcesApiService.ts`
- Migrated settings services to backend `/api/v1/settings/*`:
  - `bot-mox/src/services/settingsService.ts`
  - `bot-mox/src/services/projectSettingsService.ts`
- Removed direct Firebase access from:
  - `bot-mox/src/pages/licenses/index.tsx`
  - `bot-mox/src/components/bot/BotLicense.tsx`
  - `bot-mox/src/pages/subscriptions/index.tsx` (bot loading path)
  - `bot-mox/src/components/bot/BotSubscription.tsx` (bot loading path)
- Metric delta:
...
- Added canonical bot deletion in API v1:
  - backend route `DELETE /api/v1/bots/:id` in `proxy-server/src/modules/v1/bots.routes.js`
  - frontend helper `deleteBot` in `bot-mox/src/services/botsApiService.ts`
- Migrated project table page from direct Firebase listeners to API-first subscriptions:
  - `bot-mox/src/pages/project/index.tsx`
...
- Migrated subscription domain backend access to API-first:
  - `bot-mox/src/services/subscriptionService.ts` now uses `/api/v1/resources/subscriptions`
...
- Updated hook to remove Firebase dynamic imports:
  - `bot-mox/src/hooks/useSubscriptions.ts` now loads bot metadata via `botsApiService`
...
- Migrated settings domain services to API-first storage access:
  - `bot-mox/src/services/apiKeysService.ts` now uses `/api/v1/settings/{api_keys|proxy|notifications/events}`
  - `bot-mox/src/services/themeService.ts` now uses `/api/v1/settings/theme`
- Migrated proxy data service to backend API:
  - `bot-mox/src/services/proxyDataService.ts` now uses `resourcesApiService` + `botsApiService`
  - proxy CRUD and bot map subscriptions no longer call RTDB directly
- Migrated bot detail read paths to API-first:
  - `bot-mox/src/pages/bot/index.tsx` now uses `subscribeBotById`
  - `bot-mox/src/components/bot/BotSummary.tsx` now resolves linked resources via `/api/v1/resources/*`
  - `bot-mox/src/components/bot/BotProxy.tsx` now loads/saves proxy assignment via `/api/v1/resources/proxies`
- Migrated VM delete dependency resolver to API-first:
  - `bot-mox/src/services/vmDeleteContextService.ts` now uses `fetchBotsMap` + `fetchResources`
- Metric delta:
...
- Validation:
  ...
  - no dedicated `check:types` script exists in current `bot-mox/package.json` (typecheck covered by build step `tsc -b`)

## Latest progress snapshot (2026-02-10, continuation #5)
- Removed remaining direct RTDB usage from bot/page UI components:
  - `bot-mox/src/components/bot/BotAccount.tsx`
  - `bot-mox/src/components/bot/BotCharacter.tsx`
  - `bot-mox/src/components/bot/BotPerson.tsx`
  - `bot-mox/src/components/bot/BotSchedule.tsx`
  - `bot-mox/src/components/bot/BotLifeStages.tsx`
...
- Migrated workspace calendar/kanban service layer to canonical API:
  - `bot-mox/src/services/workspaceService.ts` now uses `/api/v1/workspace/{calendar|kanban}` with polling subscriptions.
- Metric delta:
  ...
  - remaining RTDB-bound files are now concentrated in service/hook core:
    ...
    - `bot-mox/src/services/financeService.ts`
    - `bot-mox/src/services/vmService.ts`
    - `bot-mox/src/services/vmSettingsService.ts`
    - `bot-mox/src/hooks/useFirebaseData.ts`
    - `bot-mox/src/hooks/useVMLog.ts`

## Latest progress snapshot (2026-02-10, continuation #7)
- Backend API v1 expanded for parity and domain migration:
  - added `proxy-server/src/modules/v1/finance.routes.js`:
    - `/api/v1/finance/operations` CRUD
    - `/api/v1/finance/daily-stats`
    - `/api/v1/finance/gold-price-history`
  - expanded `proxy-server/src/modules/v1/infra.routes.js` to full Proxmox/SSH parity:
  ...
  - router registration updated in `proxy-server/src/modules/v1/index.js`.
  - RTDB path map extended in `proxy-server/src/repositories/rtdb/paths.js` (`finance`, `logs`).
- Frontend API-first migration completed for remaining service/hook hotspots:
  - `bot-mox/src/services/notesService.ts` -> `/api/v1/workspace/notes` (polling subscriptions, legacy block compatibility kept).
  - `bot-mox/src/services/financeService.ts` -> `/api/v1/finance/*` + `/api/v1/settings/*`.
  - `bot-mox/src/services/vmService.ts` -> `/api/v1/infra/*` + `/api/v1/bots/*` for `upsertBotVM`.
  - `bot-mox/src/services/vmSettingsService.ts` -> `/api/v1/settings/vmgenerator*`.
  - `bot-mox/src/hooks/useFirebaseData.ts` migrated to generic API polling resolver.
  - `bot-mox/src/hooks/useVMLog.ts` persistence migrated to `/api/v1/settings/vmgenerator/task_logs`.
...
- Metric delta:
  ...
  - direct `firebase/database` imports in entire `bot-mox/src`: `0` files.
- Validation:
...
- Frontend technical debt reduction highlights:
  - `bot-mox/src/components/finance/{FinanceSummary,UniversalChart,GoldPriceChart}.tsx`:
    - moved inline render-components to top-level typed components (static-components compliance).
  - `bot-mox/src/pages/{licenses,datacenter,bot,notes}.tsx`:
  ...
  - `bot-mox/src/components/layout/{Sidebar,Header,ResourceTree}.tsx`:
...
- Lifecycle API unification:
  ...
  - added RTDB paths for lifecycle logs/archive:
    - `archive`
    - `logs/bot_lifecycle`
    - in `proxy-server/src/repositories/rtdb/paths.js`.
- Frontend lifecycle migration complete:
  - `bot-mox/src/services/botLifecycleService.ts` now uses `/api/v1` lifecycle routes.
  - removed remaining frontend Firestore usage:
    - `bot-mox/src/utils/firebase.ts` no longer initializes/exports Firestore.
    - `bot-mox/src` has no `firebase/firestore` imports.
- Dependency alignment:
  - removed `@refinedev/kbar` from frontend app/deps.
  - removed `<RefineKbarProvider/>` and `<RefineKbar/>` from `bot-mox/src/App.tsx`.
  - `npm ls @refinedev/antd antd react react-router-dom @refinedev/react-router-v6` returns clean tree (no `ELSPROBLEMS`).
- Performance baseline update:
  - introduced `manualChunks` strategy in `bot-mox/vite.config.ts`:
...
- Backend modular split advanced for IPQS domain:
  ...
  - added canonical v1 routes:
    - `proxy-server/src/modules/v1/ipqs.routes.js`
    - mounted via `proxy-server/src/modules/v1/index.js` at `/api/v1/ipqs/*`
- Legacy compatibility now delegates IPQS to shared service layer:
  - `GET /api/status`
  - `POST /api/check-ip`
  - `POST /api/check-ip-batch`
  - handlers in `proxy-server/src/legacy-app.js` now call `ipqsService` (no duplicated inline IPQS logic).
...
- Validation:
  - syntax checks are green:
    - `node --check proxy-server/src/modules/ipqs/service.js`
    - `node --check proxy-server/src/modules/v1/ipqs.routes.js`
    - `node --check proxy-server/src/modules/v1/index.js`
...
- Backend modular split advanced for WoW names domain:
  ...
  - added canonical v1 route:
    - `proxy-server/src/modules/v1/wow-names.routes.js`
    - mounted via `proxy-server/src/modules/v1/index.js` at `/api/v1/wow-names`.
- Legacy compatibility now delegates WoW names to shared service layer:
  - `GET /api/wow-names` in `proxy-server/src/legacy-app.js`.
- Legacy monolith cleanup:
...
- Validation:
  - syntax checks are green:
    - `node --check proxy-server/src/modules/wow-names/service.js`
    - `node --check proxy-server/src/modules/v1/wow-names.routes.js`
    - `node --check proxy-server/src/modules/v1/index.js`
    - `node --check proxy-server/src/legacy-app.js`

## Latest progress snapshot (2026-02-10, continuation #12)
- Frontend domain decomposition wave advanced for lifecycle UI hotspot:
  - split `bot-mox/src/components/bot/BotLifeStages.tsx` into modular subcomponents:
...
- Backend infra service-layer unification completed for proxmox/ssh HTTP routes:
  - added shared infra domain service:
    - `proxy-server/src/modules/infra/service.js`
  - added legacy compatibility router using the same service:
    - `proxy-server/src/modules/infra/legacy-routes.js`
...
- Frontend hotspot decomposition advanced for layout navigation tree:
  - refactored `bot-mox/src/components/layout/ResourceTree.tsx` into modular structure:
...
- File-size gate progress:
  - `bot-mox/src/components/layout/ResourceTree.tsx`: `982 -> 399` lines (target `<= 400` achieved).
- Validation:
...
- Frontend hotspot decomposition advanced for account domain:
  - refactored `bot-mox/src/components/bot/BotAccount.tsx` into container-oriented component.
...
- File size gate progress:
  - `bot-mox/src/components/bot/BotAccount.tsx`: `1193 -> 384` lines (target `<= 400` achieved).
- Validation:
  - frontend lint is green (`npm run -s lint` in `bot-mox`).
  - frontend build is green (`npm run -s build` in `bot-mox`).
...
- VM operations ws branch extracted from legacy monolith:
  ...
  - `proxy-server/src/legacy-app.js` now delegates VM queue ws logic via:
...
- UI proxy websocket upgrade branch extracted from legacy monolith:
  ...
  - `proxy-server/src/legacy-app.js` now delegates upgrade routing through:
...
- UI proxy HTTP fallback branch extracted from legacy monolith:
  ...
  - `proxy-server/src/legacy-app.js` now delegates:
...
- Frontend hotspot decomposition executed for project page:
  - `bot-mox/src/pages/project/index.tsx` converted to container-focused composition.
...
- File-size gate progress:
  - `bot-mox/src/pages/project/index.tsx`: `820 -> 298` lines (target `<= 400` achieved).
...
- VM service auth/settings helper branch extracted from legacy monolith:
  ...
  - `proxy-server/src/legacy-app.js` now delegates VM service settings + TinyFM/SyncThing auth/resolver logic to shared helper factory:
    - `createUiServiceAuth(...)`.
- Monolith reduction metric:
...
- File-size gate progress:
  - `bot-mox/src/pages/datacenter/index.tsx`: `769 -> 375` lines (target `<= 400` achieved).
- Behavior parity retained:
...
- Datacenter presentational decomposition wave #2 completed:
  - split content-map UI into modular sections:
    - `bot-mox/src/pages/datacenter/content-map.tsx`
    - `bot-mox/src/pages/datacenter/content-map-types.ts`
    - `bot-mox/src/pages/datacenter/content-map-sections.tsx`
    - `bot-mox/src/pages/datacenter/content-map-sections-secondary.tsx`
- File-size control progress:
  - `bot-mox/src/pages/datacenter/content-map.tsx`: `471 -> 115`.
...
- Bot account sections decomposition completed:
  - `bot-mox/src/components/bot/account/sections.tsx` converted to barrel-only exports.
  - extracted modules:
...
- Infra connectors + proxy stack extraction completed:
  ...
  - `proxy-server/src/legacy-app.js` now delegates shared:
...
- Monolith reduction metric:
  - `proxy-server/src/legacy-app.js`: `256 -> 249` lines (`< 250` target achieved).
- Quality gates updated:
...
- Frontend unification iteration completed for duplicated table row-actions:
  - added shared UI primitives:
    - `bot-mox/src/components/ui/TableActionButton.tsx`
    - `bot-mox/src/components/ui/TableActionButton.css`
  ...
  - removed local duplicate style in:
    - `bot-mox/src/components/finance/FinanceTransactions.css` (`.action-btn*`).
...
- Frontend unification wave #4 completed for licenses/notes/subscription action controls:
  - migrated to shared `TableActionButton`/`TableActionGroup` in:
    - `bot-mox/src/pages/licenses/index.tsx`
    - `bot-mox/src/components/bot/BotSubscription.tsx`
    - `bot-mox/src/components/notes/NoteSidebar.tsx`
    - `bot-mox/src/components/notes/NoteEditor.tsx`
- Refactor objective achieved in this continuation:
...
- Frontend unification wave #6 completed for schedule template row-actions:
  - migrated in `bot-mox/src/components/schedule/ScheduleGenerator.tsx`:
...
- Shared action primitive generalized and adopted for additional non-table controls:
  - `bot-mox/src/components/ui/TableActionButton.tsx` enhanced with optional `buttonType`/`buttonSize`.
  - `bot-mox/src/components/ui/TableActionButton.css` scoped styling to text variant.
  - migrated delete actions to shared component in:
    - `bot-mox/src/pages/settings/ThemeSettingsPanel.tsx`
    - `bot-mox/src/components/bot/account/generator-sections.tsx`
- Refactor objective achieved in this continuation:
...
- Bot summary split stabilization completed:
  ...
  - validated stable composition contract in:
    ...
    - `bot-mox/src/components/bot/summary/sections.tsx`
- File-size control progress:
  - `bot-mox/src/components/bot/BotSummary.tsx`: `> 400 -> 236` lines (target `<= 400` confirmed).
- Validation:
  - root `npm run -s check:all` is green.

## Latest progress snapshot (2026-02-10, continuation #40)
- Frontend hotspot decomposition executed for subscriptions page:
  - `bot-mox/src/pages/subscriptions/index.tsx` converted to container-focused composition.
  - extracted modules:
...
- File-size control progress:
  - `bot-mox/src/pages/subscriptions/index.tsx`: `457 -> 242` lines (target `<= 400` achieved).
...
- Frontend unification wave applied for VM row actions:
  - migrated `bot-mox/src/components/vm/VMList.tsx` table action controls to shared:
    - `TableActionButton`
    - `TableActionGroup`
...
- Metrics:
  - `TableActionButton/TableActionGroup` references in `bot-mox/src`: `78`.
  - files over 400 lines in `bot-mox/src`: `14` (unchanged this continuation).
- Validation:
  - root `npm run -s check:all` is green.

## Latest progress snapshot (2026-02-10, continuation #42)
- Frontend hotspot decomposition executed for bot proxy domain:
  - `bot-mox/src/components/bot/BotProxy.tsx` converted to container-focused composition.
  - extracted modules:
...
- File-size control progress:
  - `bot-mox/src/components/bot/BotProxy.tsx`: `587 -> 232` lines (target `<= 400` achieved).
...
- Frontend hotspot decomposition executed for schedule timeline module:
  - extracted repeated timeline view blocks from:
    - `bot-mox/src/components/schedule/TimelineVisualizer.tsx`
  - into:
    - `bot-mox/src/components/schedule/timeline/TimelineHeader.tsx`
    - `bot-mox/src/components/schedule/timeline/TimelineScale.tsx`
- File-size control progress:
  - `bot-mox/src/components/schedule/TimelineVisualizer.tsx`: `422 -> 359` lines (target `<= 400` achieved).
- Consolidated metrics:
...
- Frontend hotspot decomposition executed for character domain:
  - `bot-mox/src/components/bot/BotCharacter.tsx` converted to container-focused composition.
  - extracted modules:
...
- File-size control progress:
  - `bot-mox/src/components/bot/BotCharacter.tsx`: `700 -> 299` lines (target `<= 400` achieved).
- Validation:
  - root `npm run -s check:all` is green.

## Latest progress snapshot (2026-02-10, continuation #45)
- Frontend hotspot decomposition executed for bot page orchestration:
  - `bot-mox/src/pages/bot/index.tsx` converted to route/container orchestration module.
...
- File-size control progress:
  - `bot-mox/src/pages/bot/index.tsx`: `482 -> 163` lines (target `<= 400` achieved).
- Consolidated metrics:
...
- Frontend hotspot decomposition executed for licenses page:
  - `bot-mox/src/pages/licenses/index.tsx` converted to container-focused composition.
  - extracted modules:
...
- File-size control progress:
  - `bot-mox/src/pages/licenses/index.tsx`: `704 -> 273` lines (target `<= 400` achieved).
- Validation:
  - root `npm run -s check:all` is green.

## Latest progress snapshot (2026-02-10, continuation #47)
- Frontend hotspot decomposition executed for workspace calendar page:
  - `bot-mox/src/pages/workspace/calendar/index.tsx` converted to container-focused composition.
  - extracted modules:
...
- File-size control progress:
  - `bot-mox/src/pages/workspace/calendar/index.tsx`: `488 -> 255` lines (target `<= 400` achieved).
- Consolidated metrics:
...
- Frontend hotspot decomposition executed for bot person domain:
  - `bot-mox/src/components/bot/BotPerson.tsx` converted to container-focused composition.
  - extracted modules:
...
- File-size control progress:
  - `bot-mox/src/components/bot/BotPerson.tsx`: `573 -> 178` lines (target `<= 400` achieved).
- Notes:
...
- Frontend hotspot decomposition executed for bot subscription domain:
  - `bot-mox/src/components/bot/BotSubscription.tsx` converted to container-focused composition.
  - extracted modules:
...
- File-size control progress:
  - `bot-mox/src/components/bot/BotSubscription.tsx`: `405 -> 172` lines (target `<= 400` achieved).
- Consolidated metrics:
...
- Frontend hotspot decomposition executed for VM settings form domain:
  - `bot-mox/src/components/vm/VMSettingsForm.tsx` converted to container-focused composition.
  - extracted modules:
...
- File-size control progress:
  - `bot-mox/src/components/vm/VMSettingsForm.tsx`: `511 -> 162` lines (target `<= 400` achieved).
- Notes:
...
- File-size control progress:
  - `bot-mox/src/components/bot/lifeStages/StagePanels.tsx`: `406 -> 379` lines (target `<= 400` achieved).
- Consolidated metrics:
...
- Frontend file-size gate cleanup executed for bot schedule module:
  - reduced `bot-mox/src/components/bot/BotSchedule.tsx` below hotspot threshold without behavior changes.
- File-size control progress:
  - `bot-mox/src/components/bot/BotSchedule.tsx`: `403 -> 399` lines (target `<= 400` achieved).
- Consolidated metrics:
...
- Frontend decomposition executed for VM delete workflow hook:
  ...
  - updated:
    - `bot-mox/src/pages/vms/hooks/useDeleteVmWorkflow.ts` to use shared contracts.
- File-size control progress:
  - `bot-mox/src/pages/vms/hooks/useDeleteVmWorkflow.ts`: `408 -> 371` lines (target `<= 400` achieved).
- Notes:
...
- Frontend decomposition executed for VM page modal orchestration:
  - extracted modal composition from:
    - `bot-mox/src/pages/vms/VMsPage.tsx`
  - into:
...
- File-size control progress:
  - `bot-mox/src/pages/datacenter/index.tsx`: `430 -> 396` lines (target `<= 400` achieved).
- Consolidated metrics:
...
- File-size control progress:
  - `bot-mox/src/components/schedule/ScheduleGenerator.tsx`: `417 -> 384` lines (target `<= 400` achieved).
- Validation:
  - root `npm run -s check:all` is green.
...
- File-size control progress:
  - `bot-mox/src/components/schedule/TimelineVisualizer.tsx`: `410 -> 308` lines (target `<= 400` achieved).
- Consolidated metrics:
...
- Post-hardening verification sweep executed:
  - `bot-mox/src/services/botLifecycleService.ts` confirmed API-first (`/api/v1/bots/:id/lifecycle/*`) with no Firestore usage.
  - `bot-mox/src/pages` + `bot-mox/src/components` confirmed with zero direct `firebase/database` imports.
  - backend thin-entry status confirmed:
    - `proxy-server/server.js`: 3 lines,
    - `proxy-server/src/legacy-app.js`: 224 lines.
...
- Frontend API-first consolidation completed for WoW names/IPQS:
  - `bot-mox/src/components/bot/BotCharacter.tsx` moved from direct legacy fetch to service call.
  - added `bot-mox/src/services/wowNamesService.ts` (canonical `/api/v1/wow-names`).
  - `bot-mox/src/services/ipqsService.ts` migrated to canonical `/api/v1/ipqs/status|check` via `apiClient` (no direct legacy `/api/check-ip` path usage).
- Validation:
  - root `npm run -s check:all` is green.
...
- Runtime/docs parity cleanup completed:
  - startup banner in `proxy-server/src/bootstrap/runtime.js` now points to `/api/v1/health`.
  - `proxy-server/README.md` now documents canonical endpoints first (`/api/v1/ipqs/*`, `/api/v1/wow-names`, `/api/v1/health`) with legacy compatibility note.
...
- Performance chunking refinement pass executed:
  - updated `bot-mox/vite.config.ts` with safe chunking policy.
...
- Build chunking strategy finalized for stability and better natural splitting:
  - `bot-mox/vite.config.ts` no longer forces all unmatched `node_modules` into a shared `vendor-misc` bucket.
  - manual chunk fallback now returns `undefined` and lets Rollup split by import graph.
- Validation:
  - root `npm run -s check:all` is green.
  - bundle budgets are green.
  - no file-size hotspots remain in `bot-mox/src/components + bot-mox/src/pages` (`> 400` lines = `0`).
...
- Notes editor load-path optimization completed:
  - `bot-mox/src/pages/notes/index.tsx` now lazy-loads `NoteEditor` via `React.lazy` + `Suspense`.
...
- Firebase service-account bootstrap resolution hardened:
  - `proxy-server/src/bootstrap/firebase-admin.js` now resolves `FIREBASE_SERVICE_ACCOUNT_PATH` across:
    - absolute path,
  ...
  - `proxy-server/.env.example` updated with supported path examples and repo-root default.
...
- Local-dev token parity fix completed to remove `/api/v1/*` `401` storm in bypass mode:
  - backend update (`proxy-server/src/config/env.js`):
  ...
  - frontend update (`bot-mox/src/providers/auth-provider.ts`):
  ...
  - env docs update (`bot-mox/.env.example`):
...
- Frontend pre-auth race hardening completed:
  - `bot-mox/src/services/authFetch.ts` now resolves dev fallback token directly when bypass is enabled and session token is absent at request time.
...
- Production-like local env bootstrap completed (no manual copy/paste required):
  - created `proxy-server/.env` (gitignored):
  ...
  - created `bot-mox/.env` (gitignored):
...
- Backend env load-order fix:
  - `proxy-server/src/legacy-app.js` now loads dotenv before importing `./config/env`,
...
- Dev orchestration now respects configured proxy port:
  - `start-dev.js` reads `PORT` from `proxy-server/.env` (or `.env.example`) and preflights the correct port instead of hardcoding `3001`.
...
- Dev CORS fix for Vite port drift:
  - `proxy-server/src/bootstrap/http-middleware.js` now permits `http://localhost:5174` (and `127.0.0.1:5174`) in `development`,
